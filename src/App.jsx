...
function App() {
  ...
  const checkAndNotifyThreshold = useCallback(
    (ketinggian, thresholdAtas) => {
      const now = Date.now();
      const cooldownPeriod = 5 * 60 * 1000; // 5 menit cooldown

      // Cek apakah ketinggian mencapai atau melebihi batas atas
      if (ketinggian >= thresholdAtas) {
        // Cek apakah sudah pernah kirim notifikasi dalam 5 menit terakhir
        ...
        if (
          ...
          sendLocalNotification(
            "⚠️ Peringatan Level Air!",
            `Level air tandon mencapai ${ketinggian}% (Batas: ${thresholdAtas}%)`,
            "batas-atas-air"
          );
          ...
          console.log(
            `Notifikasi batas atas dikirim: ${ketinggian}% >= ${thresholdAtas}%`
          );
        }
      } else {
        // Reset flag jika ketinggian turun di bawah batas
        if (lastNotificationRef.current.batasAtas) {
          lastNotificationRef.current.batasAtas = false;
          console.log("Reset flag notifikasi - air sudah turun");
        }
      }
    },
    [sendLocalNotification]
